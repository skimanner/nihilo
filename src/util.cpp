...
const char * const BITCOIN_CONF_FILENAME = "nihilo.conf";
const char * const BITCOIN_PID_FILENAME = "nihilod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nihilo" is a composite category enabling all Nihilo-related debug output
            if(ptrCategory->count(string("nihilo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NihiloCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NihiloCore
    // Mac: ~/Library/Application Support/NihiloCore
    // Unix: ~/.nihilocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NihiloCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NihiloCore";
#else
    // Unix
    return pathRet / ".nihilocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nihilo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override nihilo.conf
